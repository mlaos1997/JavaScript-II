...
let allCaps = [];
runners.map(runner => {
    return allCaps.push(runner.first_name.toUpperCase() + ' ' + runner.last_name.toUpperCase());
})
console.log(allCaps); 
...
runners.filter(runner => {
    if(runner.shirt_size === 'L') {
        largeShirts.push(runner.first_name + ' ' + runner.last_name + ' ' + runner.shirt_size + ' ' + runner.company_name );
    } else {
        return;
    }
})
...
// const sum = ['cat', 'dog', 'bat'].reduce(function (accumulator, currentValue, currentIndex, array) {
...
runners.filter(runner => {
    if(runner.donation > 100) {
        largeDonations.push(`${runner.first_name} has donated ${runner.donation}!`);
    } else {
        return;
    }
});
